...
from utils.ext_views_sql_builder import EXT_PROGRESS_EP_PHASE, build_ext_views_sql
...
def _migrate_legacy_favorites_to_scoped(ui_state: dict | None = None) -> dict:
    """
    Однократно переносит legacy favorites в favorites_by_db.

    В старом формате избранное лежало в общем ключе `favorites`, что приводило
    к показу чужого набора после переключения между разными server|database.
    """
    _ui = dict(ui_state or _load_ui_state())
    _by_db = _ui.get("favorites_by_db")
    # Если scoped-данные уже есть, больше ничего не мигрируем:
    # иначе legacy-набор может повторно копироваться в новые сигнатуры.
    if isinstance(_by_db, dict) and _by_db:
        return _ui

    _legacy = _ui.get("favorites")
    if not isinstance(_legacy, dict) or not _legacy:
        return _ui

    _sig = _ui.get("db_signature") or "|"
    _save_ui_state(
        {
            "favorites_by_db": {_sig: dict(_legacy)},
            # Помечаем legacy как мигрированный, чтобы исключить повторные копии.
            "favorites": {},
        }
    )
...
def _reset_connection_dependent_state(new_db_signature: str) -> None:
    """
    Сбрасывает состояние мастера, завязанное на конкретную БД (сервер + имя БД).
    Вызывать при смене пары Server|Database относительно сохранённого ui_state.
    """
    _reset_graph_state()
    st.session_state.gen_fact_table_db = None
    st.session_state.gen_assessment = None
    st.session_state.gen_ext_views_sql = None
    st.session_state.pop("gen_columns_cache", None)
    st.session_state.pop("_guid_viz_path", None)
    st.session_state.pop("_graph_viz_path", None)
    try:
        _old_a = st.session_state.get("gen_analyzer")
        if _old_a is not None:
            _old_a.close()
    except Exception:
        pass
    st.session_state.gen_analyzer = None
    st.session_state.gen_relationship_builder = None
    st.session_state.gen_db_signature = new_db_signature
    try:
        st.cache_data.clear()
    except Exception:
        pass
    ...
    _save_ui_state(
        {
            "gen_fact_table_db": None,
            "gen_graph_hash": None,
            "db_signature": new_db_signature,
        }
    )
...
def _reset_graph_state():
    ...
    st.session_state.gen_relationships_path_codes_source = None
...
_saved_ui = _migrate_legacy_favorites_to_scoped(_saved_ui)


def _render_connection_error(info: dict, prefix: str = "Не удалось подключиться.") -> None:
    """
    Показывает пользователю понятную ошибку подключения и короткую подсказку.
    Техническую деталь оставляем в отдельном expander, чтобы не перегружать UI.
    """
    if not info:
        st.error(prefix)
        return
...
def _handle_runtime_db_error(exc: Exception, action_text: str) -> None:
    """
    Единая реакция на ошибки живого подключения после уже успешной авторизации.
    Сбрасываем статус подключения и просим пользователя переподключиться.
    """
    info = get_connection_error_info(exc)
    st.session_state.connection_tested = False
    st.session_state.connection_string = None
    st.session_state.gen_analyzer = None
    st.session_state["gen_connection_error"] = info
    _render_connection_error(
        info,
        prefix=f"Не удалось {action_text}. Подключение больше не считается активным.",
    )
    st.warning("Проверьте параметры подключения и нажмите `Переподключиться`.")


def _current_db_signature() -> str:
    """
    Текущая сигнатура подключения server|database.
    Используем session_state как основной источник, с fallback на текущие поля формы.
    """
    return (
        st.session_state.get("gen_db_signature")
        or get_db_signature(db_host, db_database)
    )


def _get_ui_favorites_for_db(
    ui_state: dict | None = None,
    db_signature: str | None = None,
) -> dict:
    """
    Возвращает избранные таблицы только для указанной пары server|database.

    Мягкая совместимость:
    - если есть новый контейнер favorites_by_db → берём из него;
    - если его ещё нет, используем legacy-key favorites как данные текущей БД.
    """
    _ui = ui_state if ui_state is not None else _load_ui_state()
    _sig = db_signature or _current_db_signature()
    _by_db = _ui.get("favorites_by_db")
    if isinstance(_by_db, dict):
        _scoped = _by_db.get(_sig, {})
        return dict(_scoped) if isinstance(_scoped, dict) else {}
    _legacy = _ui.get("favorites")
    # Аварийный fallback только до первой успешной миграции.
    if not isinstance(_by_db, dict) or not _by_db:
        if _sig == (_ui.get("db_signature") or "|") and isinstance(_legacy, dict):
            return dict(_legacy)
    return {}


def _save_ui_favorites_for_db(
    favorites: dict,
    db_signature: str | None = None,
) -> None:
    """Сохраняет избранные таблицы только в сегмент текущей БД."""
    _sig = db_signature or _current_db_signature()
    _ui = _load_ui_state()
    _by_db = dict(_ui.get("favorites_by_db") or {})
    _by_db[_sig] = dict(favorites or {})
    _save_ui_state({"favorites_by_db": _by_db})
...
for key, default in {
    ...
    'gen_relationships_path_codes_source': None,  # рёбра до filter_graph для «Код пути»
    'gen_table_config': {},
    'gen_excluded_fields': {},
    ...
    '_last_cred_key': None,
    'gen_db_signature': None,  # подпись server|database последнего успешного подключения
    'gen_connection_error': None,
...
# Восстанавливаем выбор источника credentials и последние ручные значения.
_saved_cred_source = _saved_ui.get("gen_cred_source")
if _saved_cred_source in ["🔒 Из переменных окружения (.env)", "✏️ Ввести вручную"]:
    st.session_state.setdefault("gen_cred_source", _saved_cred_source)
st.session_state.setdefault("gen_man_host", _saved_ui.get("gen_connection_host", ""))
st.session_state.setdefault("gen_man_db", _saved_ui.get("gen_connection_database", ""))
st.session_state.setdefault("gen_man_user", _saved_ui.get("gen_connection_username", ""))
st.session_state.setdefault("gen_man_pass", _saved_ui.get("gen_connection_password", ""))
...
if cred_source == "🔒 Из переменных окружения (.env)":
    ...
    # Принудительное переподключение: повторная проверка и сброс analyzer/кэшей данных
    if st.button(
        "🔄 Переподключиться",
        key="gen_reconnect",
        help="Проверить подключение заново и обновить клиент БД; индексы подхватятся с диска при совпадении сервера и БД.",
    ):
        st.session_state.pop("_last_cred_key", None)
        try:
            _ra = st.session_state.get("gen_analyzer")
            if _ra is not None:
                _ra.close()
        except Exception:
            pass
...
if db_host and db_database and db_username and db_password:
    if st.session_state.get('_last_cred_key') != _cred_key:
        ...
        if success:
            ...
            st.session_state.gen_connection_error = None
            _save_ui_state({
                "gen_cred_source": cred_source,
                "gen_connection_host": db_host,
                "gen_connection_database": db_database,
                "gen_connection_username": db_username,
                "gen_connection_password": db_password,
            })
        else:
            st.session_state.connection_string = None
            st.session_state.connection_tested = False
        ...
        if st.session_state.connection_tested:
            ...
            _render_connection_error(
                st.session_state.get("gen_connection_error"),
...
# Привязка UI и кэшей к паре «сервер + база» (см. также get_db_signature в utils/db_connection)
_current_db_sig = get_db_signature(db_host, db_database)
_disk_db_sig = _saved_ui.get("db_signature")
if _disk_db_sig is None:
    _save_ui_state({"db_signature": _current_db_sig})
    _saved_ui = _load_ui_state()
elif _disk_db_sig != _current_db_sig:
    _reset_connection_dependent_state(_current_db_sig)
    _saved_ui = _load_ui_state()

# Однократная очистка UI-избранного после исторической ошибки копирования
# между разными сигнатурами БД. Чистим только текущий сегмент UI-state.
if not _saved_ui.get("favorites_cleanup_v2_applied"):
    _by_db = dict(_saved_ui.get("favorites_by_db") or {})
    _by_db[_current_db_sig] = {}
    _save_ui_state(
        {
            "favorites_by_db": _by_db,
            "favorites_cleanup_v2_applied": True,
        }
    )
    _saved_ui = _load_ui_state()
st.session_state.gen_db_signature = _current_db_sig

# Создаём/переиспользуем analyzer при смене строки подключения (в т.ч. только пароля)
_cs = st.session_state.connection_string
_ga = st.session_state.get("gen_analyzer")
if _ga is None or getattr(_ga, "connection_string", None) != _cs:
    if _ga is not None:
        try:
            _ga.close()
        except Exception:
            pass
    st.session_state.gen_analyzer = StructureAnalyzer(_cs)
...
with col_build:
    if not _guid_loaded and st.button("🔨 Построить GUID-индекс", type="primary", key="gen_build_guid"):
        ...
        try:
            ...
            _handle_runtime_db_error(e, "построить GUID-индекс")
        if _guid_build_ok:
            st.rerun()
...
with col_rebuild:
    if _guid_loaded and st.button("🔄 Перестроить индекс", key="gen_rebuild_guid"):
        ...
        try:
            ...
            _handle_runtime_db_error(e, "перестроить GUID-индекс")
        if _guid_rebuild_ok:
            st.rerun()
    ...
    if not _rel_idx_loaded and _guid_loaded:
        if st.button("🔨 Построить индекс связей", type="primary", key="gen_build_rel_idx"):
            ...
            try:
                ...
                _handle_runtime_db_error(e, "построить индекс связей")
            if _ri_ok:
                st.rerun()
    ...
    if _rel_idx_loaded and _guid_loaded:
        if st.button("🔄 Перестроить индекс связей", key="gen_rebuild_rel_idx"):
            ...
            try:
                ...
                _handle_runtime_db_error(e, "перестроить индекс связей")
            if _ri_ok2:
                st.rerun()
...
with col_fs_build:
    if not _fs_loaded and st.button("🔨 Построить статистику полей", type="primary", key="gen_build_fs"):
        ...
        try:
            ...
            _handle_runtime_db_error(e, "построить статистику полей")
        if _fs_build_ok:
            st.rerun()
...
with col_fs_rebuild:
    if _fs_loaded and st.button("🔄 Перестроить статистику", key="gen_rebuild_fs"):
        ...
        try:
            ...
            _handle_runtime_db_error(e, "перестроить статистику полей")
        if _fs_rebuild_ok:
            st.rerun()
...
def _get_table_list_or_stop(_connection_string, action_text: str):
    """Безопасно получает список таблиц и при обрыве подключения останавливает workflow."""
    try:
        return _get_table_list(_connection_string)
    except Exception as exc:
        _handle_runtime_db_error(exc, action_text)
        render_context_sidebar("gen")
        st.stop()


# ═══════════════════════════════════════════════════════════════════════════
# Между секциями 4 и 5: SQL для схемы ext (представления + extended properties)
# ═══════════════════════════════════════════════════════════════════════════
st.header("··· 🧱 Набор представлений ext")
st.caption(
    "Создаёт скрипт для схемы **ext**: представления по таблицам Document / Reference / VT / Enum "
    "и метаданные в **extended properties**. Для заполнения ссылок **refs_to** / **refs_from** "
    "желательно построить индекс связей в секции 2."
)
if "gen_ext_views_sql" not in st.session_state:
    st.session_state.gen_ext_views_sql = None

_col_ext_a, _col_ext_b = st.columns([1, 2])
with _col_ext_a:
    _gen_ext_clicked = st.button(
        "Построить набор представлений ext", type="primary", key="gen_build_ext_views"
    )

# Прогресс на всю ширину под кнопкой: загрузка списка БД → пошагово по таблицам ext → EP → готово.
if _gen_ext_clicked:
    _ext_pb = st.progress(0.0, text="Подготовка: запуск…")
    try:
        _ext_pb.progress(0.06, text="Подготовка: загрузка списка таблиц из БД…")
        _tables_ext = _get_table_list_or_stop(
            st.session_state.connection_string,
            "получить список таблиц для набора ext",
        )
        _n_all = len(_tables_ext)
        _ext_pb.progress(
            0.12,
            text=f"Генерация представлений ext: получен список из {_n_all} табл., подбор кандидатов…",
        )

        def _ext_table_progress(done: int, total: int, label: str) -> None:
            """Колбэк из build_ext_views_sql: доля по обслуженным таблицам + название стадии."""
            if label == EXT_PROGRESS_EP_PHASE:
                _ext_pb.progress(
                    0.97,
                    text="Генерация метаданных: сборка extended properties в скрипт…",
                )
                return
            if total <= 0:
                _ext_pb.progress(
                    0.5,
                    text="Генерация представлений ext: таблиц для обработки не найдено",
                )
                return
            # Основная доля: 0.12 … 0.95 по мере прохода таблиц кандидатов.
            _frac = 0.12 + 0.83 * (float(done) / float(max(1, total)))
            if not label:
                _txt = f"Генерация представлений ext: к обработке {total} табл. в наборе"
            else:
                _txt = f"Генерация представлений ext: табл. {done} из {total} — {label}"
            _ext_pb.progress(min(0.96, _frac), text=_txt[:200])

        _sql_ext = build_ext_views_sql(
            analyzer,
            st.session_state.gen_structure_parser,
            _tables_ext,
            str(db_database),
            on_table_progress=_ext_table_progress,
        )
        st.session_state.gen_ext_views_sql = _sql_ext
        _ext_pb.progress(1.0, text="Завершение: скрипт ext готов")
    except Exception as _e_ext:
        st.session_state.gen_ext_views_sql = None
        _handle_runtime_db_error(_e_ext, "сформировать SQL ext")
    finally:
        _ext_pb.empty()

if st.session_state.gen_ext_views_sql:
    _sql_ext_show = st.session_state.gen_ext_views_sql
    # Полный текст не показываем в виджете — при большом скрипте перегружает WebSocket/браузер.
    st.success("Скрипт ext сформирован — скачайте файл или скопируйте в буфер ниже.")
    _col_ed, _col_ec, _col_ei = st.columns([2, 2, 3])
    with _col_ed:
        st.download_button(
            "📥 Скачать SQL",
            data=_sql_ext_show.encode("utf-8"),
            file_name="ext_views_schema.sql",
            mime="text/sql",
            use_container_width=True,
            key="gen_ext_download",
        )
    with _col_ec:
        _copy_text_clipboard_button(
            "📋 Скопировать в буфер", _sql_ext_show, element_id_suffix="ext_sql"
        )
        st.caption(f"Строк: {_sql_ext_show.count(chr(10)) + 1} | Байт: {len(_sql_ext_show):,}")
    with _col_ei:
        st.info("Пункт C (ClickHouse) в этом релизе не генерируется — только MSSQL-скрипт.")
...
all_tables = _get_table_list_or_stop(
    st.session_state.connection_string,
...
# Фильтр по типу
_section5_db_sig = get_db_signature(db_host, db_database)
_favorites = _get_ui_favorites_for_db(_saved_ui, _section5_db_sig)
...
_save_ui_state({
    'gen_fact_table_db': selected_table_db,
    'db_signature': get_db_signature(db_host, db_database),
})
...
# ─── Избранные таблицы ────────────────────────────────────────────────────
_favorites = _get_ui_favorites_for_db(_saved_ui, _section5_db_sig)  # {table_name: {"comment": str, "label": str}}
...
with col_fav_add:
    if _is_fav:
        ...
        if st.button("⭐ В избранное", key="gen_fav_add"):
            ...
            _save_ui_favorites_for_db(_favorites, _section5_db_sig)
            st.rerun()
with col_fav_info:
    if _is_fav:
...
with st.expander("📑 Массовая оценка и сводный отчёт", expanded=False):
    _bulk_db_sig = _current_db_signature()
    st.caption(
        ...
        "для каждой таблицы хранится последняя оценка в сегменте текущей пары **(server, database)**. "
        "Колонка «Избранное» хранится по БД в **favorites_by_db** внутри **output/ui_state.json**."
    )
    ...
    if (
        "_bulk_fav_snapshot" not in st.session_state
        or st.session_state.get("_bulk_fav_snapshot_sig") != _bulk_db_sig
    ):
        st.session_state["_bulk_fav_snapshot"] = _get_ui_favorites_for_db(_bulk_ui_once, _bulk_db_sig)
        st.session_state["_bulk_fav_snapshot_sig"] = _bulk_db_sig
    ...
    _rows_ui = fact_assess_store.build_rows_from_store(
        _bulk_store,
        _bulk_human,
        _fav_live,
        db_signature=_bulk_db_sig,
        legacy_allowed_tables=set(_bulk_names),
    )
    _pref_order_bulk = [
        "Избранное",
        "Таблица (техн.)",
    ...
    with _bc1:
        if st.button("Массовая оценка", type="primary", key="gen_fact_bulk_run"):
            ...
            _merged_bulk = fact_assess_store.merge_updates(
                _bulk_store,
                _upd,
                db_signature=_bulk_db_sig,
            )
            ...
            st.session_state.pop("_bulk_fav_snapshot_sig", None)
            st.rerun()
    with _bc2:
    ...
    with _bc4:
        _all_bulk_count = len(_bulk_store.get("by_table") or {})
        st.caption(f"В текущей БД: **{len(_rows_ui)}** таблиц (всего в файле: {_all_bulk_count})")
    ...
    if not _rows_ui:
        ...
        if _changed_fav:
            ...
            _save_ui_favorites_for_db(_fav_mut, _bulk_db_sig)
...
def _save_graph(
    ...
    params,
    graph_hash=None,
    relationships_pre_filter=None,
    db_signature=None,
):
    """Сохраняет граф на диск. relationships_pre_filter — снимок до filter_graph для «Код пути» при загрузке."""
    from datetime import datetime
    ...
    _sig = db_signature if db_signature is not None else get_db_signature(db_host, db_database)
    data = {
        'metadata': {
            'table': table_name,
    ...
    if relationships_pre_filter:
        data['relationships_pre_filter'] = relationships_pre_filter
...
def _load_graph(filepath):
    ...
    Returns:
        (rels_filtered, meta, rels_before_filter): отфильтрованный список, метаданные,
        копия списка из файла до filter_graph (для перечисления кодов путей в секции 10).
    """
    with open(filepath, 'r', encoding='utf-8') as f:
        data = json.load(f)
    rels = data.get('relationships', [])
    meta = data.get('metadata', {})
    # Сырые рёбра до filter_graph (новые файлы); иначе — как в файле (старые сохранения).
    rels_pre_file = data.get('relationships_pre_filter')
    rels_before_filter = list(rels_pre_file) if rels_pre_file else list(rels)
    ...
    return rels, meta, rels_before_filter
        ...
        try:
            ...
            if meta.get('graph_hash') == graph_hash and meta.get('table') == fact_table:
                if db_signature is not None:
                    _m_sig = meta.get('db_signature')
                    if _m_sig is not None and _m_sig != db_signature:
                        continue
                return str(fp)
        except Exception:
            continue
...
_cur_sig_autoload = get_db_signature(db_host, db_database)
_ui_sig_ok = _saved_ui.get('db_signature') == _cur_sig_autoload
if (
    ...
    _graph_file = _find_graph_by_hash(selected_table_db, _saved_hash, db_signature=_cur_sig_autoload)
    if _graph_file:
        st.session_state.pop("gen_loaded_cfg_filepath", None)
        st.session_state.pop("gen_loaded_cfg_filename", None)
        ...
        _ls_sig = (ls or {}).get('db_signature')
        _last_sess_ok = (
            _ls_sig == _cur_sig_autoload
            if _ls_sig
            else (_saved_ui.get('db_signature') == _cur_sig_autoload)
        )
        if (
            ls
            and ls.get('fact_table') == selected_table_db
            and ls.get('graph_hash') == _saved_hash
            and _last_sess_ok
        ):
...
_sig_list = get_db_signature(db_host, db_database)
saved_graphs = []
for g in saved_graphs_all:
    if g.get('table') != selected_table_db:
        continue
    _gm = g.get('db_signature')
    if _gm is not None and _gm != _sig_list:
        continue
    saved_graphs.append(g)
        ...
        for i, gm in enumerate(saved_graphs):
            ...
            with col_load:
                if st.button(f"📌 {label}", key=f"graph_load_{i}", use_container_width=True):
                    rels, meta, rels_pre = _load_graph(gm['filepath'])
                    st.session_state.gen_relationships_collected = rels
                    st.session_state.gen_relationships_path_codes_source = rels_pre
                    st.session_state.gen_graph_built = True
                    st.session_state.gen_graph_hash = meta.get('graph_hash')
    ...
    if (
        ...
        with st.status("🔍 Построение графа связей...", expanded=True) as _status:
            try:
                ...
                _pre_f = getattr(rb, 'last_graph_before_filter', None)
                st.session_state.gen_relationships_path_codes_source = (
                    list(_pre_f) if _pre_f is not None else list(relationships)
                )
                ...
                _save_graph(
                    selected_table_db,
                    relationships,
                    {'max_depth': max_depth, 'max_depth_up': max_depth_up},
                    graph_hash=_new_hash,
                    relationships_pre_filter=st.session_state.get('gen_relationships_path_codes_source'),
                    db_signature=get_db_signature(db_host, db_database),
                )
                _list_saved_graphs_cached.clear()
...
def _render_config_section():
    ...
    # Перечисление «Код пути» — по рёбрам до filter_graph (если заданы при построении/загрузке).
    _pcs_src = st.session_state.get('gen_relationships_path_codes_source')
    _rels_for_path_codes = relationships if _pcs_src is None else _pcs_src
    ...
    def _build_dfs_order():
        """Плоский порядок связей и отступы дерева: один старт — корневая таблица, рёбра из relationships."""
        ...
        _root = selected_table_db
        _dfs(_root, 0, 0, 0, [], {_root, _norm(_root)})
        for rel in relationships:
    ...
    # Все простые пути от корневой таблицы по графу для кодов пути (часто до filter_graph) —
    # для подписей Путь1/Путь2, ур./дп. в шапке (первый вариант) и фильтров (есть ли вариант с парой ур+дп).
    def _collect_path_variants_from_fact_root():
        ...
        for rel in _rels_for_path_codes:
            ...
            parent = rel['source_table'] if direction == 'forward' else rel['target_table']
            pn = _nl(parent)
            for pkey in (parent, pn):
                lst = _ch.setdefault(pkey, [])
                if rel not in lst:
                    lst.append(rel)
        for k in _ch:
        ...
        _seen_key = set()
        _budget = [500_000]
        _max_variants_per_rk = 32

        def _dfs_var(tb, psd, psu, path_rks, tpath, cum_by_rk):
            if _budget[0] <= 0:
                return
            for rel in _outg(tb):
                if _budget[0] <= 0:
                ...
                if direction == 'forward':
                    ...
                    csd, csu = psd, psu + 1
                if csd > _max_dd or csu > _max_du or csd + csu > _max_pl:
                    continue
                child = rel['target_table'] if direction == 'forward' else rel['source_table']
                cn = _nl(child)
                if child in tpath or cn in tpath:
                    continue
                rk = rel['relationship_key']
                ...
                _budget[0] -= 1
                _dfs_var(child, csd, csu, npath, tpath | {child, cn}, new_cum)

        _fr = selected_table_db
        _dfs_var(_fr, 0, 0, [], {_fr, _nl(_fr)}, {})

        if _budget[0] <= 0:
            st.warning(
                "Перечисление вариантов пути от корня остановлено по лимиту шагов (500000); "
                "список путей может быть неполным."
            )
        for _rk in list(_variants.keys()):
            _variants[_rk].sort(
    ...
    # Дополняем варианты для связей без записи (недостижимы по расширенному графу — редко): DFS или сирота.
    for _rel in relationships:
        _rk = _rel['relationship_key']
        if _rel_key_path_variants.get(_rk):
            continue
        if _rk in _rel_key_to_path:
            _pp = _rel_key_to_path[_rk]
            _cd = {}
            for _e in _pp:
                _cd[_e] = (_rel_key_to_cumulative_sd.get(_e, 0), _rel_key_to_cumulative_su.get(_e, 0))
            _lsd, _lsu = _cd[_pp[-1]]
            _rel_key_path_variants[_rk] = [{
                'path_rks': list(_pp),
                'cum': _cd,
                'level': _lsd - _lsu,
                'path_length': _lsd + _lsu,
            }]
        else:
            _sd, _su = _rel.get('steps_down', 0), _rel.get('steps_up', 0)
            _rel_key_path_variants[_rk] = [{
                'path_rks': [_rk],
                'cum': {_rk: (_sd, _su)},
                'level': _sd - _su,
    ...
    # Первый вариант — для шапки узла и gen_rel_key_to_path (SQL / сохранение).
    # (↓↑) по рёбрам в развёрнутом блоке берутся из variant['cum'], без глобальной подмены по ребру.
    for _rel in relationships:
        _rk = _rel['relationship_key']
        _pv = _rel_key_path_variants[_rk][0]
        _rel_key_to_level[_rk] = _pv['level']
        _rel_key_to_path_length[_rk] = _pv['path_length']
        _rel_key_to_path[_rk] = list(_pv['path_rks'])

    st.session_state.gen_rel_key_to_path = dict(_rel_key_to_path)
    ...
    def _enumerate_rel_key_path_codes():
        """
        Для каждого relationship_key отображаемой связи — множество строк ↓/↑ по всем простым путям
        от корневой таблицы факта (блок «Корневая таблица», selected_table_db) по графу _rels_for_path_codes
        (часто снимок до filter_graph). Таблица не повторяется
        на пути; без глобального visited_rk на рёбрах. Лимиты sd/su и длина пути — те же, что в UI графа.

        Returns:
            (out, by_root_ordered): множества кодов от обхода только от корневой таблицы факта (selected_table_db);
            список из одного элемента (подпись корня, dict rk→set) — для совместимости с подписью узла.
        """
        from collections import defaultdict
        ...
        # Обход по расширенному списку рёбер; опции multiselect — только для связей из relationships
        for rel in _rels_for_path_codes:
            direction = rel.get('direction', 'forward')
            parent = rel['source_table'] if direction == 'forward' else rel['target_table']
        ...
        def _outgoing_edges_for_table(tb):
            """Исходящие рёбра из вершины tb: объединение по строке и нормализованному имени (1С-варианты имён)."""
            _seen_rk = set()
            _acc = []
            for key in (tb, _norm_loc(tb)):
                for _rel in _children_loc.get(key, []):
                    _rid = _rel['relationship_key']
                    if _rid in _seen_rk:
                        continue
                    _seen_rk.add(_rid)
                    _acc.append(_rel)
        ...
        # Только корневая таблица из UI — без старта с документа-родителя ТЧ (в отличие от DFS порядка списка).
        _fact_root = selected_table_db

        def _root_label(rt: str) -> str:
            _h = sp.get_table_human_name(rt) if sp else None
            return f"{_h} (`{rt}`)" if _h else f"`{rt}`"

        def _walk_codes(rk_codes, table_name, psd, psu, code_acc, table_path_set, _walk_budget):
            if _walk_budget[0] <= 0:
                return
            for rel in _outgoing_edges_for_table(table_name):
                if _walk_budget[0] <= 0:
                    return
                rk = rel['relationship_key']
                direction = rel.get('direction', 'forward')
                ...
                if csd + csu > _max_pl:
                    continue
                # Конец простого пути: не учитывать шаг, если он ведёт в уже посещённую таблицу.
                # Иначе из корня-документа получалось «↑» в VT и «↓» обратно в документ — ложный код ↑↓
                # у ребра VT→документ, хотя такой простой путь недопустим.
                child = rel['target_table'] if direction == 'forward' else rel['source_table']
                cn = _norm_loc(child)
                ...
                _walk_budget[0] -= 1
                _walk_codes(rk_codes, child, csd, csu, new_code, table_path_set | {child, cn}, _walk_budget)

        rk_local = defaultdict(set)
        _wb = [500_000]
        _nr = _norm_loc(_fact_root)
        _walk_codes(
            rk_local,
            _fact_root,
            0,
        ...
        by_root_ordered = [(_root_label(_fact_root), dict(rk_local))]
        rk_codes = defaultdict(set)
        for _rk, s in rk_local.items():
            rk_codes[_rk] = set(s)

        if _wb[0] <= 0:
            st.warning(
                "Перечисление кодов пути остановлено по лимиту шагов (500000); "
                "фильтр «Код пути» и подписи могут быть неполными. Сузьте глубину графа."
            )

        # Только отображаемые связи; сироты — один символ по направлению ребра
        out = {}
        for rel in relationships:
            rk = rel['relationship_key']
            cs = set(rk_codes.get(rk, ()))
            if not cs:
                cs = {'↑' if rel.get('direction') == 'reverse' else '↓'}
            out[rk] = cs
        return out, by_root_ordered

    _rel_key_path_codes, _ = _enumerate_rel_key_path_codes()
    _all_path_codes = sorted(
        {c for rel in relationships for c in _rel_key_path_codes.get(rel['relationship_key'], ())}
    )

    def _format_path_codes_caption(rk: str):
        """Подпись под узлом: все простые коды пути от корневой таблицы факта (см. блок «Корневая таблица»)."""
        full = set(_rel_key_path_codes.get(rk, ()))
        if not full:
            return None
        union_str = ' · '.join(sorted(full, key=lambda s: (len(s), s)))
        return f"Коды пути от корневой таблицы: {union_str}"
    ...
    def _render_rel_node(rel, show_only_selected=False, node_number=None):
        ...
        _npaths = len(_rel_key_path_variants.get(rk) or ())
        if _npaths > 1:
            _n10, _n100 = _npaths % 10, _npaths % 100
            if _n10 == 1 and _n100 != 11:
                _pw = "путь"
            elif 2 <= _n10 <= 4 and not (12 <= _n100 <= 14):
                _pw = "пути"
            else:
                _pw = "путей"
            _path_hint = f" · {_npaths} {_pw}"
        else:
            _path_hint = ""
        header = (
            f"{num_prefix}{level_path}{status_icon} {dir_icon} **{display_name}** — полей: {n_included}/{n_total}{_path_hint}"
        )
        ...
        with _cols[1]:
            ...
            if not is_expanded:
                ...
                with row1:
                    st.markdown(header)
                    _cap_pc = _format_path_codes_caption(rk)
                    if _cap_pc:
                        st.caption(_cap_pc)
                with row2:
                ...
                with col_link:
                    # Все простые пути от корневой таблицы; при нескольких — Путь1, Путь2, …
                    _pvars = _rel_key_path_variants.get(rk) or []
                    for _pi, _var in enumerate(_pvars):
                        if len(_pvars) > 1:
                            st.caption(f"Путь{_pi + 1}: ур.{_var['level']} дп.{_var['path_length']}")
                        _vcum = _var.get('cum') or {}
                        for edge_idx, edge_rk in enumerate(_var.get('path_rks') or [rk], start=1):
                            edge_rel = _rk_to_rel.get(edge_rk)
                            if not edge_rel:
                                continue
                            edge_dir = edge_rel.get('direction', 'forward')
                            arrow = "→" if edge_dir == 'forward' else "←"
                            _esd, _esu = _vcum.get(edge_rk, (edge_rel.get('steps_down', 0), edge_rel.get('steps_up', 0)))
                            edge_steps = f" (↓{_esd} ↑{_esu})" if (_esd or _esu) else ""
                            st.caption(
                                f"{edge_idx}. 🔗 `{edge_rel['source_table']}`.`{edge_rel['field_name']}` {arrow} "
                                f"`{edge_rel['target_table']}`._IDRRef{edge_steps}"
                            )
    ...
    # Уникальные уровни и длины пути: объединение по всем вариантам; без вариантов — первичная пара (ур./дп.)
    _lvl_union = set()
    _plen_union = set()
    for _r in _sorted_rels:
        _rk0 = _r['relationship_key']
        _vars0 = _rel_key_path_variants.get(_rk0) or []
        if _vars0:
            for _v in _vars0:
                _lvl_union.add(_v['level'])
                _plen_union.add(_v['path_length'])
        else:
            _lvl_union.add(_rel_key_to_level.get(_rk0, 0))
            _plen_union.add(_rel_key_to_path_length.get(_rk0, 0))
    _all_levels = sorted(_lvl_union) if _lvl_union else [0]
    _all_path_lengths = sorted(_plen_union) if _plen_union else [0]
    ...
    _valid_lv = set(_all_levels)
    _gl_raw = st.session_state.get('gen_filter_levels', _all_levels)
    st.session_state.gen_filter_levels = [x for x in _gl_raw if x in _valid_lv] or list(_all_levels)
    _gla_raw = st.session_state.get('gen_filter_levels_applied', _all_levels)
    st.session_state.gen_filter_levels_applied = [x for x in _gla_raw if x in _valid_lv] or list(_all_levels)
    _valid_plen = set(_all_path_lengths)
    _gpl_raw = st.session_state.get('gen_filter_path_lengths', _all_path_lengths)
    st.session_state.gen_filter_path_lengths = [x for x in _gpl_raw if x in _valid_plen] or list(_all_path_lengths)
    _gpla_raw = st.session_state.get('gen_filter_path_lengths_applied', _all_path_lengths)
    st.session_state.gen_filter_path_lengths_applied = [
        x for x in _gpla_raw if x in _valid_plen
    ] or list(_all_path_lengths)
        ...
        with _pl_ms:
            st.multiselect(
                ...
                help=(
                    ...
                    "а не по отдельным числам из разных вариантов."
                ),
            )
        with _pl_btn:
            st.caption("")
    ...
    with _pc_ms:
        st.multiselect(
            ...
            help=(
                ...
                "У одной связи может быть несколько кодов (разные простые пути от этого корня — см. подпись под узлом). "
                "После «Применить»: связь остаётся, если множество её кодов пересекается с любым из выбранных (ИЛИ по опциям). "
                "Пустой выбор после «Применить» — фильтр по коду отключён. "
                "Разные несовпадающие наборы в multiselect не гарантируют непересекающиеся списки, если у связи несколько кодов. "
                "Отобрать связи с единственным кодом «↓» этим фильтром нельзя — смотрите подпись «Коды пути от корневой таблицы» у узла."
            ),
        )
    with _pc_btn:
        st.caption("")
    ...
    st.caption(
        "Код пути: перечисление только от корневой таблицы (как в блоке «Корневая таблица»). "
        "Под узлом — все такие коды; фильтр — пересечение с выбранным набором (не режим «единственный код»)."
    )
    ...
    def _rk_matches_level_path_filters(_rk):
        """Связь проходит, если существует вариант пути от корня с парой (ур, дп) в применённых фильтрах."""
        _vars = _rel_key_path_variants.get(_rk) or []
        if not _vars:
            return (
                _rel_key_to_level.get(_rk, 0) in _filter_by_level
                and _rel_key_to_path_length.get(_rk, 0) in _filter_by_path
            )
        return any(
            _v['level'] in _filter_by_level and _v['path_length'] in _filter_by_path
            for _v in _vars
        )

    _rels_to_show = _sorted_rels
    _rels_to_show = [r for r in _rels_to_show if _rk_matches_level_path_filters(r['relationship_key'])]
    ...
    if _search_str:
        ...
        def _build_searchable_text_for_rel(r):
            """Собирает весь текст узла для поиска: имя таблицы, все варианты цепочек рёбер, человеческие имена полей."""
            direction = r.get('direction', 'forward')
            ...
            _rk_s = r['relationship_key']
            _edge_paths = []
            for _pv in _rel_key_path_variants.get(_rk_s, ()):
                _pr = _pv.get('path_rks') or ()
                if _pr:
                    _edge_paths.append(_pr)
            if not _edge_paths:
                _edge_paths = [_rel_key_to_path.get(_rk_s, [_rk_s])]
            for edge_path in _edge_paths:
            ...
            with st.spinner("Отрисовка графа конфигурации..."):
                try:
                    ...
                    def _build_viz_paths():
                        ...
                        # Один старт — корневая таблица факта (как список связей и коды пути), без второго корня по нормализованному имени.
                        _start_tbl = selected_table_db
                        _dfs(_start_tbl, [], {_start_tbl, _norm_root})
